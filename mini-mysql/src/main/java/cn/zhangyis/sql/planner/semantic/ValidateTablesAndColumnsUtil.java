...
//import cn.zhangyis.sql.parser.SQLLexer;
...
//import cn.zhangyis.sql.planner.semantic.scope.SqlValidatorNamespace;
//import cn.zhangyis.sql.planner.semantic.scope.SqlValidatorScope;
//import cn.zhangyis.sql.planner.semantic.scope.TableNamespace;
...
//
//    /**
//     * 展开单个星号列，并处理重复字段名：自动添加别名（如 "name1", "name2"）
//     */
...
//
//        // 收集已存在字段名（用于去重、重命名）
//        for (SelectStatement.SelectItem item : expandedItems) {
//            String name = item.getAliasOrName(); // 假设这个方法返回字段名或别名
//            if (name != null) {
//                fieldNames.add(name.toLowerCase());
//                nameCountMap.put(name.toLowerCase(), 1);
//            }
//        }
//
...
//                    String uniqueName = getUniqueName(baseName, fieldNames, nameCountMap);
//
//                    ColumnExpression colExpr = new ColumnExpression(starExpr.getTableAlias(), baseName);
//                    colExpr.setColumnType(column.getType());
//
//                    expandedItems.add(new SelectStatement.SelectItem(colExpr, uniqueName.equals(baseName) ? null : uniqueName));
//                }
//            }
//        } else {
//            // 非限定星号：*
...
//                        String uniqueName = getUniqueName(baseName, fieldNames, nameCountMap);
//
//                        ColumnExpression colExpr = new ColumnExpression(nsName, baseName);
//                        colExpr.setColumnType(column.getType());
//
//                        expandedItems.add(new SelectStatement.SelectItem(colExpr, uniqueName.equals(baseName) ? null : uniqueName));
//                    }
...
//
//    /**
//     * 根据已有字段名生成唯一别名，如 name → name1 → name2 ...
//     */
//    private String getUniqueName(String baseName, Set<String> fieldNames, Map<String, Integer> nameCountMap) {
//        String lowerBase = baseName.toLowerCase();
//        if (!fieldNames.contains(lowerBase)) {
//            fieldNames.add(lowerBase);
//            nameCountMap.put(lowerBase, 1);
//            return baseName;
//        }
//
//        int count = nameCountMap.getOrDefault(lowerBase, 1);
//        String newName;
//        do {
//            newName = baseName + count;
//            count++;
//        } while (fieldNames.contains(newName.toLowerCase()));
//
//        nameCountMap.put(lowerBase, count);
//        fieldNames.add(newName.toLowerCase());
//        return newName;
//    }
//
//
...
//
//            // 比较表达式的结果类型始终为布尔型
//            binExpr.setColumnType(FiledType.BOOLEAN);
...
//
//            // 逻辑表达式的结果类型始终为布尔型
//            logExpr.setColumnType(FiledType.BOOLEAN);
...
//
//            // 推导函数参数的类型
//            List<Expression> args = funcExpr.getArguments();
...
//
//            // 设置函数返回类型
//            funcExpr.setColumnType(inferFunctionReturnType(funcExpr));
//        } else if (expr instanceof LiteralExpression) {
//            LiteralExpression literal = (LiteralExpression) expr;
//            // 字面量的类型根据其值确定
//            if (literal.isNumber()) {
//                literal.setColumnType(ExpressionType.LITERAL);
//            } else if (literal.isString()) {
//                literal.setColumnType(ExpressionType.LITERAL);
//            } else if (literal.isBoolean()) {
//                literal.setColumnType(ExpressionType.LITERAL);
//            } else if (literal.isNull()) {
//                literal.setColumnType(FiledType.NUll);
//            }
...
//            for (Expression valueExpr : inExpr.getValues()) {
...
//                // SUM和AVG的返回类型取决于参数类型
//                List<Expression> arg = funcExpr.getArguments();
...
//
//        // === 1. CatalogManager验证（权威验证） ===
//        Column catalogColumn = scope.findColumn(tableName, columnName);
//        if (catalogColumn == null) {
//            throw new SemanticException.ColumnNotFoundException(null, columnName);
//        }
//
//        Table table = catalogManager.getTable(catalogColumn.getTable().getName());
//        if (table == null) {
//            throw new SemanticException.TableNotFoundException(catalogColumn.getTable().getName());
//        }
...
//        } else if (expr instanceof FunctionExpression) {
//            FunctionExpression funcExpr = (FunctionExpression) expr;
//            if (funcExpr.getArguments() != null) {
//                for (Expression argument : funcExpr.getArguments()) {
...
//                // 算术运算
//                case SQLLexer.TokenType.PLUS:
//                    return evaluateArithmeticAdd(left, right);
//                case SQLLexer.TokenType.MINUS:
...
//                for (Expression argument : funcExpr.getArguments()) {
//                    containsAggregate = containsAggregate && containsAggregateFunction(argument);
//                }

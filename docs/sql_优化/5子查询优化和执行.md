...
/* 关联子查询（l_suppkey 是关联项） */
SELECT * FROM lineitem WHERE l_partkey IN (SELECT ps_partkey FROM partsupp WHERE ps_suppkey = l_suppkey);
```

PolarDB-X子查询支持绝大多数的子查询写法，具体参见[SQL使用限制](https://help.aliyun.com/zh/polardb/polardb-for-xscale/limits-on-database-development#concept-2115351 "本文介绍了数据库开发过程中的使用限制。")。
...
从以下子查询去关联化的示例中，可以看到执行计划使用JOIN代替了子查询。
...
如下示例由于`OR l_partkey < 50`的存在，导致子查询无法被去关联化，因而采用了迭代执行，这种情形下，建议改写SQL去掉子查询的OR条件。

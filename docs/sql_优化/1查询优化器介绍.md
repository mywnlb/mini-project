查询优化器负责优化逻辑计划从而输出物理计划，其主要阶段包含查询改写和计划枚举。本文将介绍查询优化器的基本原理，以及什么是关系代数运算符、查询改写（RBO阶段）和查询计划枚举（CBO阶段）。

PolarDB-X接收到一条SQL后的执行过程大致如下：![查询优化器](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9940092361/p334751.png)
...
![关系代数算子](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9940092361/p334755.png)
...
![树状图](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9940092361/p334778.png)
...
-   物理算子下推
    ...
    | **优化规则** | **描述** |
        | --- | --- |
    | 谓词下推或列裁剪 | 将Filter及Project物理算子下推至存储层MySQL执行，过滤掉不需要的行和列。 |
    | JOINClustering | 将JOIN按照拆分方式及拆分键的等值条件进行重排和聚簇，方便下一步的JOIN下推。 |
    ...
    更多关于查询下推的信息，请参见[查询改写与下推](https://help.aliyun.com/zh/polardb/polardb-for-xscale/push-down-and-rewrite-queries#concept-2117726)。

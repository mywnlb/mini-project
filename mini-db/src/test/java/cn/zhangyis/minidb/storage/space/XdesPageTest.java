...
class XdesPageTest extends BaseStorageTest {
    ...
    void testConstructor_FromPage() throws Exception {
        try (MiniTransaction mtr = new MiniTransaction(bufferPool)) {
            ...
            // 为了测试，需要手动设置页号为16384（实际使用时不会这样做）
            // 这里我们创建一个 PageId 为 16384 的页面
            PageId pageId = PageId.of(SPACE_ID, 16384);
    ...
    void testGetLocalExtentRange() throws Exception {
        ...
        XdesPage xdesPage3 = new XdesPage(pageId3);
    ...
    void testInitializeRange() throws Exception {
        try (MiniTransaction mtr = new MiniTransaction(bufferPool)) {
            ...
            // 只初始化 Extent 256-260（前5个）
            xdesPage.initializeRange(mtr, 256, 260);

            // 验证前5个被初始化
            for (int extentNo = 256; extentNo <= 260; extentNo++) {
                ExtentDescriptor ext = xdesPage.getXdesEntry(extentNo);
    ...
    void testInitializeRange_InvalidRange() throws Exception {
        try (MiniTransaction mtr = new MiniTransaction(bufferPool)) {
            ...
            // page 16384 管理 Extent 256-511，不能初始化 Extent 100
            assertThrows(IllegalArgumentException.class, () -> {
                xdesPage.initializeRange(mtr, 100, 105);
            });

            // 也不能初始化跨页的范围（256属于page 16384，512属于page 32768）
            assertThrows(IllegalArgumentException.class, () -> {
                xdesPage.initializeRange(mtr, 256, 512);
            });
        }
    }
    ...
    void testExtentRangeBoundaries() throws Exception {
        // 测试第一个 XDES Page（page 16384）的边界
        PageId pageId = PageId.of(SPACE_ID, 16384);
        XdesPage xdesPage = new XdesPage(pageId);

        // 能访问第一个 extent（256）
        ExtentDescriptor first = xdesPage.getXdesEntry(256);
        assertNotNull(first);
        assertEquals(256, first.getExtentNo());

        // 能访问最后一个 extent（511）
        ExtentDescriptor last = xdesPage.getXdesEntry(511);
        assertNotNull(last);
        assertEquals(511, last.getExtentNo());

        // 不能访问前一个（255，属于 page 0）
        assertThrows(IllegalArgumentException.class, () -> {
            xdesPage.getXdesEntry(255);
        });

        // 不能访问后一个（512，属于 page 32768）
        assertThrows(IllegalArgumentException.class, () -> {
            xdesPage.getXdesEntry(512);
        });
    }
    ...
    void testFullInitializationWorkflow() throws Exception {
        try (MiniTransaction mtr = new MiniTransaction(bufferPool)) {
            ...
            int[] range = xdesPage.getLocalExtentRange();
            assertEquals(256, range[0]);
            assertEquals(511, range[1]);
            ...
            // 测试修改其中一个 extent
            ExtentDescriptor ext300 = xdesPage.getXdesEntry(300);
            ext300.setState(mtr, ExtentState.FSEG);
            ext300.setSegmentId(mtr, 999L);
            ext300.allocatePage(mtr, 0);

            assertEquals(ExtentState.FSEG, ext300.getState());
            assertEquals(999L, ext300.getSegmentId());
            assertEquals(63, ext300.getFreePageCount());
            assertFalse(ext300.isEmpty());

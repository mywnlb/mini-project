 ...
 * 
 * <p>
 * 实现 InnoDB 改进的 LRU 算法 —— Midpoint Insertion Strategy。
 * 传统 LRU 的问题是全表扫描等操作会将热点页面挤出缓存，
 * InnoDB 通过 Young-Old 分区解决这个问题。
 * </p>
 * 
 * <h2>链表结构</h2>
 * 
 ...
 * <h2>防全表扫描污染的原理</h2>
 * <p>
 * 全表扫描会快速读取大量页面，但每个页面只访问一次。
 * 由于这些页面进入 Old 区后很快就被替换（还没过 1 秒），
 * 所以不会进入 Young 区污染热点数据。
 * </p>
 * 
 * <h2>InnoDB 参数对应</h2>
 * <ul>
...
public class LRUList {
     ...
     * 帧数组引用
     * <p>
     * 用于通过 frameId 访问 BufferFrame 对象。
     * </p>
     */
     ...
     * 
     * <p>
     * 优先返回 Old 区尾部的帧 (最久未使用)。
     * 如果 Old 区为空，则返回 Young 区尾部。
     * </p>
     * 
    ...
    public Integer findEvictableVictim() {
        lock.lock();
        try {
            int current = oldTail;
            while (current != -1) {
                BufferFrame frame = frames[current];
                if (!frame.isPinned() && !frame.isEvicting()) {
                    return current;
                }
                current = frame.lruPrev;
            }
     ...
     * 
     * <p>
     * 如果 Old 区比例低于目标值，将 Young 区尾部的页面移到 Old 区头部。
     * 这保证了 Old 区始终有足够的空间容纳新加载的页面。
     * </p>
     * 
     * <h3>平衡条件</h3>
     * 
     ...
     *
     * <p>
     * 这是 Lock-free LRU 的核心方法。由于 access() 只更新时间戳，
     * 不调整链表，所以需要后台线程定期根据 accessTime 重新排序。
     * </p>
     *
     * <h3>整理策略</h3>
     * <ol>
     ...
     * <p>
     * 建议: 每 100ms 调用一次（可配置）。
     * 频率越高，LRU越精确但开销越大；频率越低，开销小但短期不精确。
     * </p>
     *
     * @return 调整的页面数量
     */
    public int reorderPeriodically() {
        ...
        try {
            ...
            while (oldCurrent != -1) {
                ...
                if (frame.consumePromotionToYoungRequested()) {
     ...
     *
     * <p>
     * 检查链表顺序与 accessTime 顺序的一致性。
     * 返回值越接近 1.0，表示 LRU 越精确。
     * </p>
     *
     * @return 精确度 (0.0 - 1.0)
     */
